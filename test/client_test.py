...
import unittest
import nacos_py
from nacos_py import files
from nacos_py.listener import SubscribeListener, SimpleListenerManager
from nacos_py.timer import NacosTimer, NacosTimerManager
import time
import shutil
...
client = nacos_py.NacosClient(SERVER_ADDRESSES, namespace=NAMESPACE, username=USERNAME, password=PASSWORD)
...
class TestClient(unittest.TestCase):
    ...
    def test_server_failover(self):
        client2 = nacos_py.NacosClient("100.69.207.66:8848, %s:8848" % SERVER_1, namespace=NAMESPACE, username=USERNAME,
                                       password=PASSWORD)
        d = "test"
        g = "DEFAULT_GROUP"
        content = u"test中文"
    ...
    def test_long_pulling(self):
        client2 = nacos_py.NacosClient(SERVER_ADDRESSES, username=USERNAME, password=PASSWORD)
        d = "test1_pulling"
        g = "Group1"
        g2 = "Group2"
    ...
    def test_get_from_snapshot(self):
        client2 = nacos_py.NacosClient(SERVER_ADDRESSES, namespace=NAMESPACE, username=USERNAME, password=PASSWORD)
